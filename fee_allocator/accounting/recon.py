...
def recon_and_validate(fees: dict, fees_to_distribute: dict, timestamp_now: int, timestamp_2_weeks_ago: int,
    ...
    for fee_info in fees.values():
        assert fee_info[
                   "aura_incentives"] >= 0, f"Recon Failed: {fee_info['pool_id???']} Aura incentives of {fee_info['aura_incentives']}should be >= 0"
        aura_incentives += fee_info["aura_incentives"]
        assert fee_info[
                   "bal_incentives"] >= 0, f"Recon Failed: {fee_info['pool_id???']} Balancer incentives of {fee_info['bal_incentives']}should be >= 0"
        bal_incentives += fee_info["bal_incentives"]
        assert fee_info[
                   "fees_to_dao"] >= 0, f"Recon Failed: {fee_info['pool_id???']} Balancer incentives of {fee_info['fees_to_dao']}should be >= 0"
        fees_to_dao += fee_info["fees_to_dao"]
        assert fee_info[
                   "fees_to_vebal"] >= 0, f"Recon Failed: {fee_info['pool_id???']} Balancer incentives of {fee_info['fees_to_vebal']}should be >= 0"
        fees_to_vebal += fee_info["fees_to_vebal"]
        assert fee_info[
                   "total_incentives"] >= 0, f"Recon Failed: {fee_info['pool_id???']} Balancer incentives of {fee_info['total_incentives']}should be >= 0"
    ...
    for item in existing_data:
        if (item["periodStart"] == summary["periodStart"] and item["periodEnd"] == summary["periodEnd"]):
